...
const formatedCur = (value, locale, currency) => {
...
const calcAndPrintBalance = acc => {
  ...
  const formatedMov = formatedCur(acc.balance, acc.locale, acc.currency);
...
const calcDisplaySummary = acc => {
  ...
  const interest = acc.movements
    .filter(mov => mov > 0)
    .map(dep => (dep * acc.interestRate.toFixed(2)) / 100)
...
btnClose.addEventListener('click', e => {
  ...
  if (
    ...
    accounts.splice(
      accounts.findIndex(x => x.username === currentacc.username),
      1
    );
